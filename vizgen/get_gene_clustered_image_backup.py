...
ad_viz = sc.read_h5ad('/media/huifang/data/vizgen/HumanLiverCancerPatient1/gene_cluster_result/z0_ds1_res0.1/cluster_mata.hdf5')
...
for fov in range(0,fov_cnt):
    ...
    # for clust in order:
        ...
        for inst_cell in cluster_fov_meta_cells.index.tolist():
            ...
            cp_center_x, cp_center_y, cp_indexes = get_cp_location(cellpose_mask, int(np.mean(vg_indexes[0])), int(np.mean(vg_indexes[1])))
            if not cp_indexes:
                CP_FLAG = False

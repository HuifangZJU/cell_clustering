...
# ----------------------
# 1. Model: ResNet18 for regression
# ----------------------
...
# 2. Loss & Optimizer
# ----------------------
criterion = nn.SmoothL1Loss(beta=1.0)  # Huber # regression loss (MAE)
...
def train_model(num_epochs=10):
    ...
    for epoch in range(num_epochs+1):
        ...
        for i, (imgs, labels) in enumerate(train_loader):
            # labels are z-scored continuous values
            imgs = imgs.to(device, non_blocking=True)
            labels = labels.to(device).float().unsqueeze(1)

            optimizer.zero_grad()
            outputs = model(imgs)  # regression output
            loss = criterion(outputs, labels)
            loss.backward()
            optimizer.step()

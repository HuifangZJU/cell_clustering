...
def get_predictions(p, dataloader, model, return_features=False):
    ...
    for batch in dataloader:
        ...
        bs = images.shape[0]
        # res = model(images, sizes, forward_pass='return_all')
...
def hungarian_evaluate(subhead_index, all_predictions, class_names=None, 
    ...
    k=3
    _, preds_topk = probs.topk(k, 1, largest=True)
    reordered_preds_topk = torch.zeros_like(preds_topk)
    for pred_i, target_i in match:
        reordered_preds_topk[preds_topk == int(pred_i)] = int(target_i)
    correct_topk_binary = reordered_preds_topk.eq(targets.view(-1,1).expand_as(reordered_preds_topk))
    topk = float(correct_topk_binary.sum()) / float(num_elems)
    ...
    return {'ACC': acc, 'ARI': ari, 'NMI': nmi, 'ACC Top-'+str(k): topk, 'hungarian_match': match}

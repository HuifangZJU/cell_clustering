...
def get_model(p, pretrain_path=None):
    ...
    elif p['setup'] in ['spaformer']:
        from models.models import SpaFormerModel
        model = SpaFormerModel(backbone,in_dim=128,num_hidden=128, num_layers=1,nhead=2,feat_drop=0.6,pe='bin')
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    # Base dataset
    if p['train_db_name'] == 'vizgen':
        from data.vizgen import vizgen
        dataset = vizgen(split='train',transform=transform,ds_rate = p['ds_rate'])
    else:
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    # Base dataset
    if  p['val_db_name'] == 'vizgen':
        from data.vizgen import vizgen
        dataset = vizgen(split='test',transform=transform,ds_rate = p['ds_rate'])
    else:

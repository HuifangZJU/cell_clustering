...
def fill_memory_bank(loader, model, memory_bank):
    ...
    for i, batch in enumerate(loader):
        ...
        position_x = batch['position_x'].cuda(non_blocking=True)
        position_y = batch['position_y'].cuda(non_blocking=True)
        # sizes = batch['size'].cuda(non_blocking=True)
        # output = model(images,sizes)
        output = model(images)
        memory_bank.update(output,position_x,position_y, targets)
        if i % 100 == 0:
            print('Fill Memory Bank [%d/%d]' %(i, len(loader)))

def fill_memory_bank_with_pretrained_feature(image_memory_bank,simclr_feature):
    image_memory_bank.features = simclr_feature['feature']
    image_memory_bank.position_x = simclr_feature['position_x']
    image_memory_bank.position_y = simclr_feature['position_y']
    image_memory_bank.targets = simclr_feature['target']
    return image_memory_bank

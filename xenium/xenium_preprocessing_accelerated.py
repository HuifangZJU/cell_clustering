...
def process_single_cell(cell_id, cell_data, cell_boundaries, image_data, fixed_ratio_dir, pixel_size):
    """Process a single cell: crop the image and save."""
    # Get cell type
    ...
    half_side = side // 2
    # Ensure boundaries don't go outside the image range
    ...
    # Return information about the processed cell
    return cell_id, center_x, center_y, cell_type, fixed_ratio_path


def parallel_crop_and_save(cell_data, cell_boundaries, image_data, fixed_ratio_dir, pixel_size, n_jobs=-1):
    """Parallelize the cropping and saving process for all cells."""
    # Run the process in parallel
    results = Parallel(n_jobs=n_jobs)(
        delayed(process_single_cell)(
            cell_id, cell_data, cell_boundaries, image_data, fixed_ratio_dir, pixel_size
        ) for cell_id in cell_data['cell_id'].unique()
    )

    # Collect cell information
    cell_info = [(cell_id, cx, cy, ctype, path) for cell_id, cx, cy, ctype, path in results]
    return cell_info
...
def crop_and_save_images_with_types(image_data, cell_boundaries, cell_types, output_dir, pixel_size):
    ...
    # Run parallel cropping and saving
    cell_info = parallel_crop_and_save(cell_data, cell_boundaries, image_data, fixed_ratio_dir, pixel_size)
